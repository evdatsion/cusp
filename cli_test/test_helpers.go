...
func NewFixtures(t *testing.T) *Fixtures {
	...
	return &Fixtures{
		...
		BuildDir:      buildDir,
		RootDir:       tmpDir,
		CuspdBinary:   filepath.Join(buildDir, "cusp"),
		CuspcliBinary: filepath.Join(buildDir, "cuspcli"),
		CuspdHome:     filepath.Join(tmpDir, ".cusp"),
...
// UnsafeResetAll is cusp unsafe-reset-all
...
// AddGenesisAccount is cusp add-genesis-account
...
// CollectGenTxs is cusp collect-gentxs
func (f *Fixtures) CollectGenTxs(flags ...string) {
	cmd := fmt.Sprintf("%s collect-gentxs --home=%s", f.CuspdBinary, f.CuspdHome)
	executeWriteCheckErr(f.T, addFlags(cmd, flags), client.DefaultKeyPass)
}

// GDStart runs cusp start with the appropriate flags and returns a process
...
// GDAphelion returns the results of cusp aphelion [query]
func (f *Fixtures) GDAphelion(query string) string {
...
// ValidateGenesis runs cusp validate-genesis
